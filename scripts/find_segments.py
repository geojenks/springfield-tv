...
2. Boundaries, per anchor, from 1 fps thumbnails of the episode file:
     bezel  - frame is drawn inside the in-show TV (dark flat corners, bright centre).
              Runs bridged over gaps <= BRIDGE s (cutaways to the sofa).  Primary.
...
from scan_episodes import frames_1fps, yellow_ratio, dhash, hamming, runs_below, hms, tag_of, W, H
...
def thumb(fr, t, path):
    ...
    Image.fromarray(fr[t]).resize((W * 2, H * 2)).save(path, quality=80)
...
.m{color:#8f8}.m.no_yellow{color:#fc6}.m.anchor_only{color:#f66}
        ...
        for i, an in enumerate(anchors):
            ...
            for t in range(max(st - 10, 0), st):        # title card just before the run
                if rf[t]:
                    ...
                    method += "+ref"
                    break
            conf = {"bezel": 0.9, "no_yellow": 0.5, "anchor_only": 0.2}[method.split("+")[0]]
            if d:
            ...
            rows.append(dict(id=cid, episode=key, category=an["cat"], anchor=hms(t0),
                             ...
                             confidence=round(conf, 2), anchor_text=" / ".join(an["lines"])[:200]))
            thumbs = []
